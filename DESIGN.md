...
导航应是**“空气”**——需要时存在，不需要时消失，绝不喧宾夺主。
首页为长滚动结构，导航主要承担**“快速定位 (Anchors)”** 功能。

### 13.2 布局策略

- **位置**：Fixed Top (z-index: 100).
- **结构**：
    - **Left**: Logo (Text "YFY" or icon).
    - **Right**: Links [About] [Tech] [Impact].
- **样式**：
    - **Top State (Y=0)**: 全透明背景，Text Color 跟随 Hero 背景（白色/浅色）。
    - **Scrolled State (Y>50)**: Glassmorphism (Blur + 半透明黑底)，确保在任何内容上可读。
